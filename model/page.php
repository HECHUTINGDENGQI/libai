...
**/
   ...
   function pageData($table,$p,$num=4,$where='',$field='*',$order='id desc'){
	    ...
	    $total_page = ceil($totalnum['count']/$num);
	    // var_dump($total_page);die;
	    //总条数/当前页的多少条数据 ceil 向上取 floor 向下取
	    // echo $total_page;
	    if($p<1){ //当前页少于1
	    	$p=1;
	    }
	    ...
	    $data =  sql_list($table,$where,$order,$map,$field);


	    return $data;
	 
   }
   function pageJoinData($table1,$table2,$p,$on,$order='id desc',$field='*',$num=4,$where='',$join='INNER JOIN'){
   	   // $p = isset($_GET['p'])?$_GET['p']:1;//当前页 get url
	    // $num = 3;//每页显示条数
  ...
  **/
   ...
   function page($table,$p,$num=4,$url,$showpage=4,$where='',$field='*'){
	    ...
	    $page .= '<a href="index.php?'.$url.'&p='.$prev.'">上一页</a>';
	    ...
	    for ($i=$start_page; $i <=$end_page; $i++) { 
	    	if($i==$p){
                $page .= '<a class="current" href="index.php?'.$url.'&p='.$i.'">'.$i.'</a>';
	    	}else{
	    		$page .= '<a href="index.php?'.$url.'&p='.$i.'">'.$i.'</a>';
	    	}
	    ...
	    $page .= '<a href="index.php?'.$url.'&p='.$next.'">下一页</a>';
	    $page .= '<a href="index.php?'.$url.'&p='.$total_page.'">尾页</a>';
	    // echo '<br>';
	    return $page;
   }
  ...
  **/
   ...
   function page_home($table,$p,$num=4,$url,$showpage=4,$where='',$field='*'){
	    ...
	    //$page .= '<li><a href="index.php?'.$url.'&p=1">首页</a></li>';
	    $page .= '<li><a href="index.php?'.$url.'&p='.$prev.'#jb">上一页</a></li>';
	    ...
	    for ($i=$start_page; $i <=$end_page; $i++) { 
	    	if($i==$p){
                $page .= '<li><a class="current" href="index.php?'.$url.'&p='.$i.'">'.$i.'</a></li>';
	    	}else{
	    ...
	    $page .= '<li><a href="index.php?'.$url.'&p='.$next.'#jb">下一页</a></li>';
	    //$page .= '<li><a href="index.php?'.$url.'&p='.$total_page.'">尾页</a></li>';
	    // echo '<br>';
	    return $page;
   }